...
- They are rows of the gem optimizer's model: ReforgeOptimizeRequest.stat_constraints (copied from BulkSettings by sim/web/bulk.go and ui/sim/wasm/bulk_sim/reforge.ts; also set by sim.ts so the reforge cache key covers them) become gap-to-base bounds in sim/core/reforge_optimizer/stat_constraints.go. A stat no variable moves is decided on the base stats. An infeasible model sets ReforgeOptimizeResult.infeasible_stat_constraints and the pre-pass flags the candidate (BulkGearCandidate.skipped_by_constraints) instead of falling back to its original gear; dedupe keeps flagged candidates.
- The gate stays the final stats: sim/core/bulk/constraints.go and ui/sim/wasm/bulk_sim/constraints.ts run ComputeStats on every remaining candidate after the pre-pass (progress stage BulkSimStageConstraints) and drop failures, so optimizer-off runs and cache-restored gear are covered. BulkSimResult.skipped_by_constraints counts both kinds; the tab shows it above the results.
- A constraint row on a stat suppresses the cap refinement row for that stat (checkCaps skips stats that already have a row).
